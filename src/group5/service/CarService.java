...
public class CarService {
	...
	/*
	 * this method provides dropdown options
	 */
	public Vector<String> getOptionsFromAllCars(String filterName, ArrayList<Car> list) {
		...
		if (filterName.equalsIgnoreCase("category")) {
			return new Vector<String>(list.stream().map(vehicle -> vehicle.getCarCategory().toString()).collect(Collectors.toSet()));
		}
	...
	/*
	 * this method performs the search operation and results the matching records
	 */

	public ArrayList<Car> getSearchedData(String searchKeyWord, ArrayList<Car> completeCarList) {
		
		ArrayList<Car> resultList = (ArrayList<Car>) completeCarList.stream()
				  .filter( ( vehicle -> vehicle.getMake().contains(searchKeyWord)
	...
	/*
	 * this method performs the sorting based on the option selected by the user and returns the matching records
	 */
	 public ArrayList<Car> getFilteredData(HashMap<String, String> searchParamsMap,ArrayList<group8.Car> completeList ) { 
			 ...
			 */
	        	 if(key.equalsIgnoreCase("category")) {
	        		 ...
	        		 filterListOnCategory = (ArrayList<Car>) completeList.stream()
	        				 					  .filter(vehicle -> vehicle.getCarCategory().toString().equalsIgnoreCase(searchParamsMap.get(key)))
	        				 					  .collect(Collectors.toList());
